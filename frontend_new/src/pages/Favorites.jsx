...
const Favorites = () => {
    ...
    const removeFavorite = async (songId) => {
        ...
        try {
            await api.delete(`/music/favorites/${songId}`);
            ...
            <div className="space-y-4">
                {filtered.length === 0 ? <p className="text-center text-slate-400 py-8">No tienes favoritos aún</p> : filtered.map((song) => (
                    <Card key={song.id} className="flex items-center justify-between gap-3">
                        ...
                        <button onClick={() => removeFavorite(song.id)} className="p-2 text-slate-400 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
                    </Card>
                ))}
            </div>
        </div>
